...
const DarkModeSwitcher = observer(({ isDisable }: DarkModeSwitcherProps) => {
  const [colorMode, setColorMode] = useColorMode();
  useEffect(() => {
    () => {
      const theme = stores.appStore.applicationSetting?.theme as any;
  ...
  return (
    <li>
      <Tooltip
        tooltipText={`${
          stores.appStore.applicationSetting?.theme === 'light'
            ? 'Dark Mode'
            : 'Light Mode'
        }`}
      >
        <label
          className={`relative m-0 block h-7 w-14 rounded-full  ${
            colorMode === 'dark' ? 'bg-bgGrey' : 'bg-stroke'
          }`}
        >
          <input
            type='checkbox'
